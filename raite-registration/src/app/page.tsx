...
export default async function HomePage() {
  ...
  // Helper to find ranking by place (supporting ties)
    ...
    <div className="flex flex-col min-h-screen bg-background text-foreground relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto w-full grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="space-y-10 text-center lg:text-left">
            <motion.div 
              ...
              className="relative w-32 h-16 mx-auto lg:mx-0"
            >
              <Image
                src="/psite.png"
                alt="PSITE Logo"
            ...
            <div className="space-y-6">
              ...
              >
                <span className="bg-gradient-to-r from-[#0038A8] via-[#CE1126] to-[#FCD116] bg-clip-text text-transparent">
                  RAITE 2026
                </span>
              </motion.h1>
              <motion.p 
                ...
                className="max-w-xl mx-auto lg:mx-0 text-xl md:text-2xl font-black tracking-tight text-foreground leading-tight"
              >
                CTRL + NEXT: <br />
                <span className="text-muted-foreground font-medium text-lg md:text-xl italic">
                  Empowering Students as Future-Ready Digital Innovators and Technology Leaders
                </span>
              </motion.p>
            </div>
            ...
            >
              {isGuest && (
                <Button asChild size="lg" className="h-16 px-10 rounded-xl text-lg font-bold shadow-2xl shadow-primary/30">
                  <Link href="/sign-in">Get Started</Link>
                </Button>
              )}

              {isNewUser && (
                <Button size="lg" className="h-16 px-10 rounded-xl text-lg font-bold shadow-2xl shadow-primary/30">
              ...
              {isAdminOrCoach && (
                <>
                  <Button size="lg" className="h-16 px-10 rounded-xl text-lg font-bold shadow-2xl shadow-primary/30">
                    <Link href="/participants/register">Register Participants</Link>
                    </Button>
                  <Button size="lg" variant="secondary" className="h-16 px-10 rounded-xl text-lg font-bold">
                    <Link href="/register/step-1">Registration</Link>
                  </Button>
                </>
              )}
              ...
              {isParticipant && (
                <>
                  {!hasActiveRegistration ? (
                    ...
                    <Button size="lg" className="h-16 px-10 rounded-xl text-lg font-bold shadow-2xl shadow-primary/30">
                      <Link href="/registrations/my">My Status</Link>
                    </Button>
                  )}
                </>
              )}
              
              <Button variant="ghost" size="lg" className="h-16 px-8 rounded-xl text-lg font-bold group border border-border/50">
              ...
              <div className="flex flex-col items-center text-center space-y-6">
                ...
                <div className="w-full">
                  {missionStartDate ? (
                    ...
                    <p className="text-xl font-bold text-white/50 tracking-widest">SCHEDULE PENDING</p>
                  )}
                </div>
        ...
        <div className="container mx-auto max-w-7xl">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
            ...
            <Card className="md:col-span-12 p-8 md:p-12 border border-border bg-card/80 overflow-hidden relative shadow-sm">
              ...
              <div className="flex items-end justify-center gap-4 md:gap-8 w-full max-w-5xl mx-auto pt-10">
                ...
                <div className="flex flex-col items-center flex-1">
                  <div className="w-16 h-16 rounded-full bg-secondary flex items-center justify-center mb-6 border border-border shadow-sm">
                  ...
                  <div className="w-full bg-secondary/50 rounded-t-xl p-6 md:p-8 text-center min-h-48 md:min-h-64 flex flex-col justify-between border-t border-x border-border">
                    <div className="space-y-3">
                      {secondPlace.length > 0 ? secondPlace.map((e, i) => (
                        <div key={i} className="space-y-1">
                          <p className="font-bold text-foreground text-sm md:text-xl line-clamp-2">{e.university}</p>
                          {i < secondPlace.length - 1 && <div className="h-px bg-border/50 w-1/2 mx-auto" />}
                        </div>
                      )) : (
                        <p className="text-muted-foreground italic text-sm">TBD</p>
                      )}
                    </div>
                    <div className="mt-4">
                      <p className="text-[10px] font-black uppercase tracking-[0.2em] text-primary">1st Runner Up</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col items-center flex-1 -mt-8 md:-mt-12">
                  <div className="w-20 h-20 rounded-full bg-primary flex items-center justify-center mb-6 border-4 border-white shadow-md">
                  ...
                  <div className="w-full bg-primary rounded-t-2xl p-8 md:p-10 text-center min-h-64 md:min-h-80 flex flex-col justify-between border-t border-x border-primary relative shadow-lg">
                    <div className="space-y-4">
                      {firstPlace.length > 0 ? firstPlace.map((e, i) => (
                        <div key={i} className="space-y-1 text-white">
                          <p className="font-bold text-lg md:text-2xl leading-tight line-clamp-2">{e.university}</p>
                          {i < firstPlace.length - 1 && <div className="h-px bg-white/20 w-1/2 mx-auto" />}
                        </div>
                      )) : (
                        <p className="text-white/60 italic text-sm">TBD</p>
                      )}
                    </div>
                    <div className="mt-4">
                      <p className="text-[10px] md:text-xs text-white font-black uppercase tracking-[0.2em]">Grand Champion</p>
                    </div>
                  </div>
                </div>
                ...
                {/* 3rd Place */}
                <div className="flex flex-col items-center flex-1">
                  <div className="w-14 h-14 rounded-full bg-secondary flex items-center justify-center mb-6 border border-border shadow-sm">
                  ...
                  <div className="w-full bg-secondary/50 rounded-t-xl p-6 md:p-8 text-center min-h-40 md:min-h-52 flex flex-col justify-between border-t border-x border-border">
                    <div className="space-y-2">
                      {thirdPlace.length > 0 ? thirdPlace.map((e, i) => (
                        <div key={i} className="space-y-1">
                          <p className="font-bold text-foreground text-xs md:text-lg line-clamp-2">{e.university}</p>
                          {i < thirdPlace.length - 1 && <div className="h-px bg-border/50 w-1/2 mx-auto" />}
                        </div>
                      )) : (
                        <p className="text-muted-foreground italic text-sm">TBD</p>
                      )}
                    ...
                    <div className="mt-4">
                      <p className="text-[8px] md:text-[10px] text-muted-foreground font-bold uppercase tracking-widest mt-2">2nd Runner Up</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {competitionWinners.length > 0 && (
              <Card className="md:col-span-12 p-8 md:p-12 border border-border bg-card/80 overflow-hidden relative shadow-sm">
                <div className="text-center mb-10">
                  <h3 className="text-2xl md:text-3xl font-bold tracking-tight uppercase text-foreground">RAITE 2025 Competition Winners</h3>
            ...
            <Card className="md:col-span-8 p-10 flex flex-col justify-center border border-border bg-card/80 overflow-hidden shadow-sm group transition-all duration-500 relative min-h-[280px]">
              {/* Image Background (Initial State) */}
              ...
              {/* Map Background (Revealed on Hover) */}
              <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-700 z-10">
                <iframe
                  ...
                  loading="lazy"
                  src="https://maps.google.com/maps?q=Cabambangan,+Bacolor,+2001+Pampanga&t=&z=15&ie=UTF8&iwloc=&output=embed"
                ></iframe>
              ...
              <div className="flex flex-col md:flex-row items-center gap-10 w-full relative z-20 transition-transform duration-500 group-hover:translate-y-[-5px]">
                <div className="w-20 h-20 rounded-2xl bg-secondary flex items-center justify-center text-primary border border-primary/10 shrink-0 group-hover:bg-white group-hover:shadow-lg transition-all">
                ...
                <div className="space-y-2 text-center md:text-left flex-1">
                  ...
                  <p className="text-[12px] font-bold text-muted-foreground uppercase tracking-widest group-hover:text-foreground transition-colors duration-500">
              ...
              <div className="w-full">
                <div className="flex items-center justify-between mb-8">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-xl bg-secondary flex items-center justify-center text-primary border border-primary/10">
                      <Megaphone className="w-5 h-5" />
                    </div>
                    <h3 className="text-xl font-bold tracking-tight uppercase text-foreground">Broadcasts</h3>
                  </div>
                  <Link href="/announcements" className="text-[10px] font-bold text-primary hover:underline flex items-center gap-2 uppercase tracking-widest transition-all hover:translate-x-1">
      ...
      <footer className="py-16 border-t border-border bg-background text-center relative z-10">
        <div className="container mx-auto px-4">
          <p className="text-xs font-bold text-muted-foreground uppercase tracking-[0.3em] mb-6 text-foreground">
