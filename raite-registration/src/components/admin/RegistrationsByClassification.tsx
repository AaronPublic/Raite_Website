...
interface RegistrationsByClassificationProps {
  data: { name: string; count: number }[];
}

const COLORS = ["#3b82f6", "#6366f1", "#8b5cf6", "#a855f7", "#d946ef", "#ec4899", "#f43f5e", "#ef4444"];

export default function RegistrationsByClassification({ data }: RegistrationsByClassificationProps) {
  return (
                ...
                if (active && payload && payload.length) {
                  return (
                    <div className="bg-white dark:bg-gray-950 p-4 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-800">
                      ...
                      <p className="text-lg font-black text-blue-600">{payload[0].value} <span className="text-xs font-medium text-gray-500">Users</span></p>
                    </div>
                  );
                }
                return null;
